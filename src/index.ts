import { WSChannel } from "./lib/isecure.class.js";
...
const publicKeyArmored = fs.readFileSync(path.join(__dirname, "../test/test-pgp-key.pub"), "utf8").toString();
const privateKeyArmored = fs.readFileSync(path.join(__dirname, "../test/test-pgp-key.sec"), "utf8").toString();
...
async function createAndVerifyPgpSignature() {
  ...
  const message = await openpgp.createMessage({
    text: fs.readFileSync(path.join(__dirname, "../test/testfile.xml"), "utf8").toString(),
  });
...
async function main() {
  ...
  const contents = fs.readFileSync(path.join(__dirname, "../test/testfile.xml"), "utf8").toString();
