...
- Fixed a classification bug surfaced by a new end-to-end onboarding test: a lingering access token from the email-verification stage no longer re-triggers email verification on a later MFA-stage re-login (classification now keys on the access token in the current response).
